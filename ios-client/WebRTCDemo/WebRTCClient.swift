...
class WebRTCClient: NSObject, RTCPeerConnectionDelegate, RTCVideoViewDelegate {
    ...
    private var contraints: (video: Bool, audio: Bool) = (false, false)
    ...
    func setup(videoTrack: Bool, audioTrack: Bool, customFrameCapturer: Bool){
        print("set up")
        self.contraints.video = videoTrack
        self.contraints.audio = audioTrack
    ...
    private func setupLocalTracks(){
        if self.contraints.video == true {
            self.localVideoTrack = createVideoTrack()
        }
        if self.contraints.audio == true {
